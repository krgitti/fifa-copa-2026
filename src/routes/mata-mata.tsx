...
import { type Match } from "@/lib/worldcup-data";
          ...
          return (
            <section key={key}>
              <div className="mb-3 flex items-center gap-2">
                <h2
                  className={`text-xl font-bold tracking-tight ${key === "final" ? "text-accent" : ""}`}
                >
                  {label}
                </h2>
              ...
              <div
                className={`grid gap-3 ${key === "final" || key === "third" ? "sm:grid-cols-1" : "sm:grid-cols-2 lg:grid-cols-3"}`}
              >
                {ms.map((m) => (
